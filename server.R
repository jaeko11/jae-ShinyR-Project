...
shinyServer(function(input, output, session) {
    ...
    start_station = reactive({
        ...
        NYCFerry_start = NYCFerry_start %>% group_by(start.station.name, 
                                               
                                               start.station.longitude, start.station.latitude) %>%
            
            summarise(count=n()) %>% arrange(desc(count))
        
        NYCFerry_start=nycferry_start[1:input$start_num, ]
    ...
    end_station = reactive({
        ...
        NYCferry_end = NYCferry_end %>% group_by(end.station.name, 
                                             
                                             end.station.longitude, end.station.latitude) %>%
            
            summarise(count=n()) %>% arrange(desc(count))
        
        NYCFerry_end=NYCFerry_end[1:input$end_num, ]
    ...
    output$avg_time = renderPlot({
        ...
        ggplot(data=nycferry, aes(x=tripduration))+
            
            geom_histogram(color='white')+xlim(c(0,75))})
    
    output$avg_age = renderPlot({ggplot(data=nycferry, aes(x=Age))+
            
            geom_histogram(color='white')+xlim(c(0,100))})
    
    output$gender_prop = renderPlot({ggplot(nycferry) + geom_bar(aes(gender_var, 
                                                                     
                                                                     y = (..count..)/sum(..count..), fill=gender_var)) + ylab("Proportion")+ xlab('Gender')})
    
    output$weekday_usage = renderPlot({
        
        ggplot(nycferry) + geom_bar(aes(x=weekday, fill=weekday)) + theme_bw() + ylab("Count")
    ...
    output$hour_usage = renderPlot({
        ...
        ggplot(nycferry) + geom_bar(aes(x=hour, fill=hour)) + theme_bw() + ylab("Count")
